...
function replaceText(word, replacement, text) {
  ...
  const ReplaceRegexp = text.replaceAll(word, replacement);
...
function findWordOccurrences(str, word) {
  ...
  // Повертаємо масив
  const regexp = RegExp(`\\b${word}\\b`, "gi");
...
function checkRegexMethods(regex) {
  ...
  const flags = [];
  if (regex.dotAll) flags.push("dotAll");

  if (regex.multiline) flags.push("multiline");

  if (regex.sticky) flags.push("sticky");
